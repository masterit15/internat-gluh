<?php
/**
 * internat-gluh functions and definitions
 *
...
function internat_gluh_setup() {
	/*
		...
		* If you're building a theme based on internat-gluh, use a find and replace
		* to change 'internat-gluh' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'internat-gluh', get_template_directory() . '/languages' );
...
function internat_gluh_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'internat-gluh' ),
			'id'            => 'sidebar-1',
...
function admin_js() {
  global $post;
	if ($post) {
    wp_enqueue_script( 'jquery-script', get_template_directory_uri() . '/libs/jquery/jquery.min.js');
